...
class Grammar {
  ...
  complexDepth: number = 0;
  complexOrder: boolean = false;
  ...
  async init() {
    // Parse grammar file
    const grammarFile = __dirname + "/../grammars/" + this.lang + ".json";
    const grammarJson = jsonc.parse((await fsp.readFile(grammarFile)).toString());
      ...
      // Go back
        ...
        // Complex terms require multi-level analyzes
          ...
          for (const d of scopes)
            if (d in this.complexScopes) term = this.complexScopes[d];
        }
...
class TokensProvider
  ...
  constructor() {
    ...
    const enabledTerms: string[] = vscode.workspace
      .getConfiguration("fuior")
      .get("highlightTerms");
    availableTerms.forEach((term) => {
      if (enabledTerms.includes(term)) this.supportedTerms.push(term);
    });
  ...
  async provideDocumentSemanticTokens(
    ...
    terms.forEach((t) => {
      if (!this.supportedTerms.includes(t.term)) return;
  ...
  async provideHover(
    ...
    if (!(uri in this.trees)) return null;
...
export function activateSyntax(context: vscode.ExtensionContext) {
  const supportedLangs: { language: string }[] = [{ language: "fuior" }];
